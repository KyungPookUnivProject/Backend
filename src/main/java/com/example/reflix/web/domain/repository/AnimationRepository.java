...
public interface AnimationRepository extends JpaRepository<Animation,Long> {
    ...
    @Query("select new com.example.reflix.web.dto.ContentsRecommendResponseDto(m.contentsId,m.Name,m.ImageUrl,m.year,m.contentsCategory,0) from Animation m  where m.contentsId IN :longs and m.year > :startDate and m.year < :endDate")
    ...
    @Query("select m.contentsId from Animation m")
    List<Long> findAllId();
}
