...
//    }
    ...
    //추천되는 콘텐츠들을 유저별로 추천콘텐츠목록에 저장
    @Transactional
    public void recomendContentsSave(List<recommendContentsDto> contentsList,Long id){
        List<recomendContents> recomendContentsList = new ArrayList<>();
        for(recommendContentsDto contents : contentsList) {
            recomendContents recomendContents1 = recomendContents.builder()
                    .contentsCategory(contents.getContentsCategory())
...
//    }
    ...
    public boolean contentLike(Long contentId,int category,userAdapter user){
            ...
            switch (category){
                case 0: category1= com.example.reflix.web.domain.category.MOVIE; break;
                case 1: category1 = com.example.reflix.web.domain.category.DRAMA;break;
                case 2: category1 = com.example.reflix.web.domain.category.ANIMATION; break;
            }
    ...
    public boolean contentDisLike(Long contentId, Long userId){
            Optional<contentLikeList> rid = contentLikeListRepository.findByContentIdAndUser(contentId,userId);
            if(rid.isPresent()) {
